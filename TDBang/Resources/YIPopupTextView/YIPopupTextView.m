...
- (id)initWithPlaceHolder:(NSString*)placeHolder maxCount:(NSUInteger)maxCount
{
    ...
    if (self) {
        ...
        _backgroundView = [[UIView alloc] initWithFrame:CGRectMake(0, 64, 320, mainHeight - 64)];
        _backgroundView.backgroundColor = [UIColor lightGrayColor];
        _backgroundView.alpha = 0;
        _backgroundView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
        
        _popupView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, mainWidth, 200)];
...
//        [_popupView addSubview:_closeButton];
        ...
        _sendButton.frame = CGRectMake(_popupView.bounds.size.width/2,
                                       _popupView.bounds.size.height-40,
                                       _popupView.bounds.size.width/2,
                                       40);
        ...
        _sendButton.titleLabel.font = [UIFont systemFontOfSize:COUNT_SIZE];
        [_sendButton addTarget:self action:@selector(handleSendComment:) forControlEvents:UIControlEventTouchUpInside];
        _sendButton.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleTopMargin;
        [_sendButton.layer setBorderColor:[UIColor colorWithWhite:220/255.0f alpha:1.0f].CGColor];
        [_sendButton.layer setBorderWidth:0.4f];
        [_popupView addSubview:_sendButton];
        ...
        _cancelButton.titleLabel.font = [UIFont systemFontOfSize:COUNT_SIZE];
        [_cancelButton addTarget:self action:@selector(handleCloseButton:) forControlEvents:UIControlEventTouchUpInside];
        _cancelButton.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleTopMargin;
        [_cancelButton.layer setBorderColor:[UIColor colorWithWhite:220/255.0f alpha:1.0f].CGColor];
        [_cancelButton.layer setBorderWidth:0.4f];
...
- (void)handleSendComment:(UIButton *)sender
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(sendComment:)]) {
        [self.delegate sendComment:self.text];
        [self dismiss];
    }
}
