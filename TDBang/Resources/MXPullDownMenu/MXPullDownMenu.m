...
- (MXPullDownMenu *)initWithArray:(NSArray *)array selectedColor:(UIColor *)color
{
    ...
    if (self) {
        ...
        CGFloat textLayerInterval = mainWidth / ( _numOfMenu * 2);
        CGFloat separatorLineInterval = mainWidth / _numOfMenu;
...
- (void)tapMenu:(UITapGestureRecognizer *)paramSender
{
    ...
    // 得到tapIndex
    NSInteger tapIndex = touchPoint.x / (mainWidth / _numOfMenu);
    for (int i = 0; i < _numOfMenu; i++) {
        if (i != tapIndex) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell.textLabel.text == [(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string]) {
        ...
        [cell.textLabel setTextColor:RGB(245, 175, 57)];
    }
...
{
    if (show) {
        tableView.frame = CGRectMake(0, self.frame.origin.y + self.frame.size.height, mainWidth, 0);
        ...
        [UIView animateWithDuration:0.2 animations:^{
            _tableView.frame = CGRectMake(0, self.frame.origin.y + self.frame.size.height, mainWidth, tableViewHeight);
        }];
    } else {
        [UIView animateWithDuration:0.2 animations:^{
            _tableView.frame = CGRectMake(0, self.frame.origin.y + self.frame.size.height, mainWidth, 0);
        } completion:^(BOOL finished) {
...
- (void)animateTitle:(CATextLayer *)title show:(BOOL)show complete:(void(^)())complete
{
    ...
    CGFloat sizeWidth = (size.width < (mainWidth / _numOfMenu) - 25) ? size.width : mainWidth / _numOfMenu - 25;
...
- (UITableView *)creatTableViewAtPosition:(CGPoint)point
{
    ...
    tableView.frame = CGRectMake(point.x, point.y, mainWidth, 0);
